...
const WhiteboardCanvas: React.FC<WhiteboardCanvasProps> = ({ content, onChange, title = "Sandbox Board", onSave }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Ignore parser errors for empty or corrupted strings
    }
  }, [content]);
  ...
  // Get mouse coordinates transformed by stage panning/zooming
  ...
  const handleMouseDown = (e: any) => {
    // If select mode, let default dragging happen
    if (tool === "select") {
  ...
  return (
    <div className="relative flex w-full h-full bg-slate-50 select-none overflow-hidden" ref={containerRef}>
          ...
          {[2, 4, 8].map((size) => (
            <button
              ...
              className={cn(
                ...
                strokeWidth === size ? "bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-slate-100" : "text-slate-500"
              )}
            >
          ...
          {(["none", "semi", "solid"] as const).map((mode) => (
            <button
              ...
              className={cn(
                ...
                fillMode === mode ? "bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-slate-100" : "text-slate-500"
              )}
            >
              {mode}
            </button>
      ...
      <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
        ...
        {selectedId && (
          <Button variant="destructive" size="icon" onClick={deleteSelected} title="Delete Selected">
            <Trash2 className="h-4 w-4" />
          </Button>
        )}
        <Button variant="outline" size="icon" onClick={clearCanvas} title="Clear Whiteboard">
          <Trash2 className="h-4 w-4 text-red-500" />
        </Button>
        <Button variant="outline" size="icon" onClick={exportAsImage} title="Export to PNG">
          <Download className="h-4 w-4" />
        </Button>
        {onSave && (
          <Button className="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold shadow" onClick={onSave}>
            Save
          </Button>
        )}
      </div>
